  ...
  try {
    ...
    if (view === "assignedToMe") {
      const tasks = await Task.find({ assigneeId: userId })
        ...
        .populate("assigneeId", "name email")
        .sort({ createdAt: -1 });
      return res.json({ assignedToMe: tasks,count: tasks.length  });
    }
    ...
    if (view === "assignedByMe") {
      const tasks = await Task.find({ creatorId: userId })
        ...
        .populate("assigneeId", "name email")
        .sort({ createdAt: -1 });
      return res.json({ assignedByMe: tasks, count: tasks.length });
    }

    // default: return both (for dashboard single call)
    const assignedToMe = await Task.find({ assigneeId: userId })
      .populate("creatorId", "name email")
      .populate("assigneeId", "name email")
      .sort({ createdAt: -1 });
    const assignedByMe = await Task.find({ creatorId: userId })
      .populate("creatorId", "name email")
      .populate("assigneeId", "name email")
      .sort({ createdAt: -1 });
...
export const getTask = async (req, res) => {
  try {
    ...
    if (
      task.creatorId._id.toString() !== uid &&
      task.assigneeId._id.toString() !== uid &&
      req.user.role !== "ADMIN"
    ) {
      return res.status(403).json({ message: "Forbidden" });
    }
...
export const updateTask = async (req, res) => {
  try {
    ...
    if (
      ...
      task.creatorId.toString() !== uid
    ) {
      return res
        .status(403)
        .json({ message: "Only creator can edit title/description/assignee" });
    }
    ...
    if (status !== undefined) {
      ...
      if (
        task.assigneeId.toString() !== uid &&
        task.creatorId.toString() !== uid
      ) {
        return res
          .status(403)
          .json({ message: "Not allowed to update status" });
      }
      task.status = status;
    }
...
export const deleteTask = async (req, res) => {
  try {
    ...
    const uid = req.user._id.toString();
    if (task.creatorId.toString() !== uid)
